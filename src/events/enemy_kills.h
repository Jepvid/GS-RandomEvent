...
// Requires: rng.h, sm64.h (gMarioState, MarioState), game/mario.h (hurt_and_set_mario_action)

// Enemies that damage Mario all carry INTERACT_DAMAGE.
// Collectibles (coins, stars, caps, warps) do not — reliable enough filter.
...
static void on_object_destroyed(IEvent *event) {
    ...
    // Only care about interactive enemies.
    if (!(e->object->oInteractType & ENEMY_INTERACT_MASK)) return;

    // ~5% explosion chance.
    if (rng_next() % 20 != 0) return;

    // Always spawn a visible explosion at the kill site.
    ...
    // Hurt Mario if he is within the blast radius.
